...
function detectFillers(text: string) {
  ...
  FILLER_WORDS.forEach(fw => {
    const m = lower.match(new RegExp(`\\b${fw}\\b`,'gi'));
    if (m) found[fw] = m.length;
  });
  return found;
}
...
const css = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  @keyframes recPulse{0%,100%{box-shadow:0 0 0 0 rgba(239,68,68,.5)}60%{box-shadow:0 0 0 10px rgba(239,68,68,0)}}
  .fadeUp{animation:fadeUp .5s ease forwards}
  .scaleIn{animation:scaleIn .25s ease forwards}
...
function ScoreRing({ score, size=140 }: { score:number; size?:number }) {
  ...
  const sw=10, r=(size-sw*2)/2, c=2*Math.PI*r;
  ...
  useEffect(()=>{
    ...
    const step=(ts:number)=>{
      ...
      setD(+(score*(1-Math.pow(1-p,3))).toFixed(1));
      if(p<1)requestAnimationFrame(step); else setD(score);
    };
    ...
    <div style={{position:'relative',width:size,height:size,flexShrink:0}}>
      <svg width={size} height={size} style={{transform:'rotate(-90deg)'}}>
        ...
        <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={col} strokeWidth={sw}
          strokeDasharray={c} strokeDashoffset={c-(d/10)*c} strokeLinecap="round"/>
      </svg>
      <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
        <span style={{fontSize:size*.24,fontWeight:900,color:col,fontFamily:'"Playfair Display",serif',lineHeight:1}}>{d}</span>
        <span style={{fontSize:size*.1,color:'rgba(255,255,255,.4)',marginTop:2}}>/10</span>
      </div>
    </div>
  );
}
...
function SeverityBadge({severity}:{severity:string}) {
  const m:Record<string,[string,string]>={low:['#22c55e','rgba(34,197,94,.1)'],medium:['#f59e0b','rgba(245,158,11,.1)'],high:['#ef4444','rgba(239,68,68,.1)']};
  const [t,bg]=m[severity?.toLowerCase()]||['#94a3b8','rgba(148,163,184,.1)'];
  return <span style={{background:bg,color:t,border:`1px solid ${t}44`,borderRadius:4,padding:'2px 8px',fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:'.05em'}}>{severity}</span>;
}

function AuthModal({authTab,setAuthTab,authName,setAuthName,authEmail,setAuthEmail,authPassword,setAuthPassword,authError,authLoading,onClose,onSignUp,onSignIn}:any) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.82)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:20,backdropFilter:'blur(10px)'}}
      ...
      <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:22,padding:32,width:'100%',maxWidth:480}} className="scaleIn">
        ...
        {authTab==='signup'&&<input value={authName} onChange={e=>setAuthName(e.target.value)} type="text" placeholder="Your name" style={inp}/>}
        ...
        <input value={authPassword} onChange={e=>setAuthPassword(e.target.value)}
          ...
          type="password" placeholder="Password (min. 6 characters)" style={{...inp,marginBottom:authError?8:14}}/>
        {authError&&<div style={{fontSize:13,color:'#ef4444',marginBottom:12,padding:'9px 12px',background:'rgba(239,68,68,.08)',borderRadius:9,border:'1px solid rgba(239,68,68,.2)'}}>{authError}</div>}
        <button className="btn" onClick={authTab==='signup'?onSignUp:onSignIn} disabled={authLoading}
          style={{width:'100%',background:`linear-gradient(135deg,${G.accent},#8b5cf6)`,color:'#fff',fontWeight:700,fontSize:15,padding:'13px',borderRadius:11,marginBottom:14,boxShadow:`0 6px 20px ${G.accentGlow}`,opacity:authLoading?.7:1}}>
        ...
        <p style={{fontSize:12,color:G.muted,textAlign:'center' as any}}>
          ...
          {authTab==='signin'&&<button className="btn" onClick={()=>setAuthTab('signup')} style={{background:'none',color:G.accent,fontSize:12,textDecoration:'underline',border:'none',padding:0}}>Sign up free</button>}
        </p>
      </div>
    </div>
  );
...
function Footer({onPrivacy}:{onPrivacy:()=>void}) {
  return (
    <footer style={{textAlign:'center',padding:'24px 20px',borderTop:`1px solid ${G.border}`,marginTop:40}}>
      <p style={{fontSize:12,color:G.muted}}>
        © {new Date().getFullYear()} LionMountain Pictures LLC · SpeakCoachAI
        {' · '}<button onClick={onPrivacy} style={{background:'none',border:'none',color:G.accent,cursor:'pointer',fontSize:12,textDecoration:'underline',padding:0}}>Privacy Policy</button>
        {' · '}<button onClick={()=>window.open('/tos.html','_blank')} style={{background:'none',border:'none',color:G.accent,cursor:'pointer',fontSize:12,textDecoration:'underline',padding:0}}>Terms of Service</button>
      </p>
    </footer>
  );
}

// ─── RECORD SCREEN — outside App, owns ALL recording state incl. timer ───────
// This means App NEVER re-renders due to timer ticks → no video flicker
...
type RecordScreenProps = {
  ...
  onShowAuth: ()=>void;
  onShowUpgrade: ()=>void;
  onTranscribeAndAnalyze: (blob:Blob, duration:number, pauses:any[])=>void;
  onAnalyze: ()=>void;
  onReset: ()=>void;
...
function RecordScreen(props: RecordScreenProps) {
  const { transcribing, transcript, apiError, isPaid, remaining, isLoggedIn,
          onHome, onShowAuth, onShowUpgrade, onTranscribeAndAnalyze,
          onAnalyze, onReset, setApiError, openaiKey, canAnalyze } = props;

  // ── All recording state lives HERE — App never re-renders for these ──
  const [mode, setMode] = useState('record');
  const [recording, setRecording] = useState(false);
  const [recTime, setRecTime] = useState(0);        // ← timer ticks cause RecordScreen re-render ONLY
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch(e:any) { setCamError(e.message||'Camera not accessible'); }
    })();
    return () => { active = false; };
  }, [mode]);
  ...
  const startRecording = async () => {
    ...
    if (!canAnalyze) { onShowUpgrade(); return; }
    setCamError(null); setPauses([]); pausesRef.current = []; audioChunksRef.current = [];
    try {
      ...
      // Timer lives in RecordScreen — App doesn't know about it
      timerRef.current = setInterval(() => {
        setRecTime(t => { if (t>=59) { doStop(); return 60; } return t+1; });
      }, 1000);
      // Pause detection
      ...
      const check = () => {
        ...
        if (avg>8) {
          ...
          if (gap>2000&&pausesRef.current.length<20) { pausesRef.current.push({duration:gap}); setPauses([...pausesRef.current]); }
          lastSoundRef.current = now;
        }
        pauseFrameRef.current = requestAnimationFrame(check);
      };
      pauseFrameRef.current = requestAnimationFrame(check);
    } catch { setCamError('No camera/microphone access. Please allow permissions.'); }
  };
  ...
  const doStop = (): Promise<Blob> => {
    ...
    return new Promise<Blob>(resolve => {
      ...
      if (!mr || mr.state==='inactive') { resolve(new Blob(audioChunksRef.current,{type:'audio/webm'})); return; }
  ...
  const handleStopAndTranscribe = async () => {
    ...
    const capturedPauses = [...pausesRef.current];
    const blob = await doStop();
    onTranscribeAndAnalyze(blob, duration, capturedPauses);
  };
  ...
  const transcribeUpload = async () => {
    ...
    } catch(e:any) { setApiError('Transcription error: '+e.message); }
    finally { setUploadTranscribing(false); }
  };
  ...
  return (
    <div style={{minHeight:'100vh',width:'100%',background:G.bg,display:'flex',flexDirection:'column'}}>
      <div style={{padding:'16px 28px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:`1px solid ${G.border}`}}>
        <button className="btn" onClick={()=>{doStop();onHome();}} style={{color:G.muted,background:'none',fontSize:14}}>← Home</button>
        ...
        {!isPaid&&isLoggedIn&&<div style={{fontSize:13,color:G.muted,background:G.card,padding:'4px 12px',borderRadius:8,border:`1px solid ${G.border}`}}>{remaining} free left</div>}
      </div>
        ...
        <div style={{display:'flex',background:G.card,border:`1px solid ${G.border}`,borderRadius:12,padding:4,gap:4,width:'fit-content'}}>
          {[['record','🎥 Record'],['upload','📁 Upload']].map(([m,l])=>(
            <button key={m} className="btn" onClick={()=>setMode(m)}
              style={{background:mode===m?`linear-gradient(135deg,${G.accent},#8b5cf6)`:'none',color:mode===m?'#fff':G.muted,border:'none',fontWeight:600,padding:'7px 18px',borderRadius:8,fontSize:13}}>{l}</button>
          ))}
        </div>

        {mode==='record' ? (
          <div style={{display:'grid',gridTemplateColumns:'1fr 340px',gap:18,alignItems:'start'}}>
            {/* Video box — stable, never affected by timer ticks in App */}
            <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:18,overflow:'hidden',position:'relative',aspectRatio:'16/10' as any}}>
              ...
              {!recording&&!camError&&!transcribing&&!streamRef.current&&(
                <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(7,8,15,.88)',flexDirection:'column',gap:10}}>
                  <div style={{fontSize:42}}>🎥</div>
                  <div style={{color:G.muted,fontSize:14}}>Camera starts when you record</div>
                </div>
              )}
              {transcribing&&(
                <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(7,8,15,.92)',flexDirection:'column',gap:14}}>
              ...
              {camError&&(
                <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(7,8,15,.9)',flexDirection:'column',gap:10,padding:24,textAlign:'center' as any}}>
              ...
              {recording&&(
                <>
                  <div style={{position:'absolute',top:12,left:12,display:'flex',alignItems:'center',gap:7,background:'rgba(0,0,0,.72)',padding:'5px 11px',borderRadius:8}}>
                  ...
                  <div style={{position:'absolute',top:12,right:12,background:'rgba(0,0,0,.72)',padding:'4px 10px',borderRadius:7,fontSize:12,color:G.muted}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:14}}>
              <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:18,padding:22,textAlign:'center'}}>
                {!recording&&!transcript&&!transcribing ? (
                  <>
                    <div style={{fontSize:13,color:G.muted,marginBottom:14,lineHeight:1.65}}>Speak for 60 seconds on any topic — introduce yourself, pitch an idea, or give a short speech.</div>
                    <button className="btn" onClick={startRecording}
                      style={{background:`linear-gradient(135deg,#ef4444,#dc2626)`,color:'#fff',fontWeight:700,fontSize:15,padding:'13px 28px',borderRadius:11,width:'100%',boxShadow:'0 5px 18px rgba(239,68,68,.3)'}}>🔴 Start Recording</button>
                  </>
                ) : recording ? (
                  <>
                    ...
                    <button className="btn" onClick={handleStopAndTranscribe}
                      style={{background:G.surface,border:`1px solid ${G.border}`,color:G.text,fontWeight:700,fontSize:14,padding:'10px 22px',borderRadius:10,width:'100%'}}>⏹ Stop & Transcribe</button>
                  </>
                ) : null}
              </div>
              ...
              {(transcript||transcribing)&&(
                <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:14,padding:16}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:9}}>
                    <span style={{fontSize:11,fontWeight:700,color:transcribing?G.accent:'#22c55e',textTransform:'uppercase' as any,letterSpacing:'.08em'}}>
              ...
              {transcript&&!recording&&!transcribing&&(
                <div className="fadeUp">
                  {apiError&&<div style={{fontSize:13,color:'#ef4444',marginBottom:10,padding:'10px 14px',background:'rgba(239,68,68,.08)',borderRadius:10,border:'1px solid rgba(239,68,68,.2)',lineHeight:1.5}}>{apiError}</div>}
                  <button className="btn" onClick={onAnalyze}
                    style={{background:`linear-gradient(135deg,${G.accent},#8b5cf6)`,color:'#fff',fontWeight:700,fontSize:15,padding:'13px',borderRadius:11,width:'100%',boxShadow:`0 6px 20px ${G.accentGlow}`}}>🔍 Analyze Now →</button>
                  <button className="btn" onClick={onReset} style={{background:'none',color:G.muted,fontSize:13,width:'100%',padding:'8px',marginTop:6}}>↺ Record again</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 320px',gap:18,alignItems:'start'}}>
            <div style={{display:'flex',flexDirection:'column',gap:14}}>
              <div onClick={()=>fileRef.current?.click()}
                ...
                style={{background:drag?G.accentSoft:G.card,border:`2px dashed ${drag?G.accent:G.border}`,borderRadius:18,padding:44,textAlign:'center' as any,cursor:'pointer',transition:'all .2s'}}>
                <input ref={fileRef} type="file" accept="video/*,audio/*" style={{display:'none'}} onChange={e=>handleFile(e.target.files?.[0]||null)}/>
                {uploadFile
                  ?<><div style={{fontSize:36,marginBottom:10}}>🎬</div><div style={{fontWeight:700,marginBottom:4}}>{uploadFile.name}</div><div style={{color:G.muted,fontSize:13}}>{uploadDur}s · {(uploadFile.size/1024/1024).toFixed(1)} MB</div></>
                  :<><div style={{fontSize:36,marginBottom:10}}>📁</div><div style={{fontWeight:700,marginBottom:6}}>Drop your video or audio here</div><div style={{color:G.muted,fontSize:13}}>MP4, MOV, WebM, MP3, M4A · min. 60 seconds</div></>}
              </div>
              {uploadFile&&(
            ...
            <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:18,padding:22}}>
              ...
              {!uploadFile
                ...
                : <>
                    ...
                    {apiError&&<div style={{fontSize:13,color:'#ef4444',marginBottom:12,padding:'9px 12px',background:'rgba(239,68,68,.08)',borderRadius:9}}>{apiError}</div>}
                    <button className="btn" onClick={()=>{
                      if(!isLoggedIn){onShowAuth();return;}
                      if(!canAnalyze){onShowUpgrade();return;}
                      if(!uTranscript||uTranscript.trim().split(/\s+/).length<15){setApiError('Please add a transcript first.');return;}
                      onTranscribeAndAnalyze(new Blob([],{type:'audio/webm'}),uploadDur,[]);
                    }} style={{background:`linear-gradient(135deg,${G.accent},#8b5cf6)`,color:'#fff',fontWeight:700,fontSize:14,padding:'12px',borderRadius:10,width:'100%',boxShadow:`0 5px 18px ${G.accentGlow}`,marginTop:6}}>🔍 Analyze →</button>
                  </>
              }
            </div>
          </div>
...
// ─── MAIN APP — never re-renders due to timer ticks ──────────────────────────
...
export default function App() {
  ...
  const [pausesData, setPausesData] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_e, session) => {
      if (session?.user) { setUser(session.user); fetchProfile(session.user.id); }
      else { setUser(null); setProfile(null); setAnalysesUsed(0); }
    });
  ...
  const fetchProfile = async (uid: string) => {
    ...
    if (data) { setProfile(data); setAnalysesUsed(data.analyses_used||0); }
  };

  const createProfile = async (uid: string, name: string) => {
    const { data } = await supabase.from('profiles').insert({user_id:uid,name,analyses_used:0,is_paid:false,plan:'free',source:'web'}).select().single();
    if (data) { setProfile(data); setAnalysesUsed(0); }
  };
  ...
  const handleSignUp = async () => {
    ...
    if (data.user) await createProfile(data.user.id,authName);
    setShowAuth(false); setAuthLoading(false);
  };
  ...
  // Called by RecordScreen when recording stops — App transcribes + stores pauses
  const handleTranscribeAndAnalyze = async (blob: Blob, duration: number, pauses: any[]) => {
    setTranscribing(true); setApiError(null);
    setRecDuration(duration); setPausesData(pauses);
    try {
      const fd = new FormData();
      ...
      if (!res.ok){const e=await res.json();throw new Error(e.error?.message||'Transcription error');}
      const data = await res.json();
      setTranscript(data.text||'');
    } catch(e:any){setApiError('Transcription failed: '+(e.message||'Unknown'));}
    finally{setTranscribing(false);}
  };

  const runAnalysis = async (text:string, duration:number, pauses:any[]) => {
    ...
    try {
      ...
      const wpm = Math.round(words/Math.max(duration/60,.5));
      const longPauses = pauses.filter(p=>p.duration>3000).length;
      const prompt = `You are a world-class rhetoric coach. Analyze this speech transcript with precision and honesty.\n\nTRANSCRIPT:\n"${text}"\n\nMETRICS:\n- Duration: ${duration}s\n- Words: ${words}\n- WPM: ${wpm} (ideal: 130–160)\n- Filler words: ${fillerStr}\n- Long pauses (>3s): ${longPauses}\n- Total pauses: ${pauses.length}\n\nRespond ONLY with valid JSON, no markdown:\n{"overallScore":<1-10>,"summary":"<3 sentences>","categories":[{"name":"Speech Flow","score":<1-10>,"icon":"🗣️","comment":"<1 sentence>"},{"name":"Tone & Expression","score":<1-10>,"icon":"🎵","comment":"<1 sentence>"},{"name":"Clarity & Structure","score":<1-10>,"icon":"🧩","comment":"<1 sentence>"},{"name":"Speaking Pace","score":<1-10>,"icon":"⏱️","comment":"<1 sentence with ${wpm} WPM>"},{"name":"Confidence & Presence","score":<1-10>,"icon":"💪","comment":"<1 sentence>"}],"issues":[{"type":"Filler Words","severity":"<low|medium|high>","detail":"<assessment>"},{"type":"Pauses & Hesitations","severity":"<low|medium|high>","detail":"<assessment>"},{"type":"Sentence Structure","severity":"<low|medium|high>","detail":"<observation>"},{"type":"Persuasiveness","severity":"<low|medium|high>","detail":"<assessment>"}],"tips":["<Tip 1>","<Tip 2>","<Tip 3>","<Tip 4>","<Tip 5>"]}`;
      const res = await fetch('https://api.openai.com/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${OPENAI_KEY}`},body:JSON.stringify({model:'gpt-4o',max_tokens:1500,messages:[{role:'system',content:'Respond with valid JSON only.'},{role:'user',content:prompt}]})});
      const data = await res.json();
      if (data.error) throw new Error(data.error.message);
      const parsed = JSON.parse(data.choices[0].message.content.replace(/```json|```/g,'').trim());
      timers.forEach(clearTimeout); setAnalysisStep(4);
      const nc = analysesUsed+1; setAnalysesUsed(nc);
      if (user&&profile) await supabase.from('profiles').update({analyses_used:nc}).eq('user_id',user.id);
      setTimeout(()=>{setResults(parsed);setScreen('results');},700);
    } catch(e:any){timers.forEach(clearTimeout);setApiError('Analysis failed: '+(e.message||'Unknown'));setScreen('record');}
  };

  const handleAnalyze = () => {
    if (!user){setShowAuth(true);return;}
    if (!transcript||transcript.trim().split(/\s+/).length<15){setApiError('Not enough speech detected. Please speak for at least 20–30 seconds.');return;}
    runAnalysis(transcript, recDuration, pausesData);
  };

  const reset = () => {
    setScreen('record'); setTranscript(''); setPausesData([]);
    setRecDuration(60); setResults(null); setApiError(null);
  };
    ...
    <>
      ...
      {screen==='home' && (
        <div style={{width:'100%',background:G.bg}}>
          <nav style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'18px 40px',borderBottom:`1px solid ${G.border}`,background:`${G.surface}dd`,backdropFilter:'blur(12px)',position:'sticky',top:0,zIndex:100}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              ...
              <button className="btn" onClick={()=>{if(!user){setAuthTab('signup');setShowAuth(true);}else if(!canAnalyze)setShowUpgrade(true);else setScreen('record');}}
                style={{background:`linear-gradient(135deg,${G.accent},#8b5cf6)`,color:'#fff',fontSize:14,fontWeight:700,padding:'8px 20px',borderRadius:10}}>Try Now →</button>
            </div>
          </nav>
          <div style={{maxWidth:880,margin:'0 auto',padding:'72px 40px 56px',textAlign:'center'}}>
            <div style={{display:'inline-flex',alignItems:'center',gap:8,background:G.accentSoft,border:`1px solid ${G.accent}44`,borderRadius:20,padding:'5px 14px',marginBottom:26,fontSize:13,color:G.accent,fontWeight:600}}>✨ AI-Powered Speech & Presence Analysis</div>
            <h1 style={{fontSize:'clamp(34px,6vw,68px)',fontFamily:'"Playfair Display",serif',fontWeight:900,lineHeight:1.08,marginBottom:22,background:`linear-gradient(135deg,${G.text} 0%,rgba(241,243,249,.5) 100%)`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>Speak with conviction.<br/>Every single time.</h1>
            <p style={{fontSize:17,color:G.muted,lineHeight:1.75,maxWidth:540,margin:'0 auto 36px'}}>Record 60 seconds and instantly discover what to improve. AI analyzes your tone, pauses, filler words, and presence — with personalized, actionable feedback.</p>
            <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
              <button className="btn" onClick={()=>{if(!user){setAuthTab('signup');setShowAuth(true);}else if(!canAnalyze)setShowUpgrade(true);else setScreen('record');}}
                style={{background:`linear-gradient(135deg,${G.accent},#8b5cf6)`,color:'#fff',fontSize:16,fontWeight:700,padding:'14px 30px',borderRadius:12,boxShadow:`0 8px 28px ${G.accentGlow}`}}>🎙️ Start for Free</button>
          ...
          <div style={{maxWidth:960,margin:'0 auto',padding:'0 40px 72px',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:16}}>
            {[{icon:'🎯',title:'Precise Scoring',text:'1–10 score across 5 categories: Flow, Tone, Clarity, Pace, and Presence.'},{icon:'🐛',title:'Catch Your Mistakes',text:'Filler words, long pauses, and hesitations automatically detected.'},{icon:'💡',title:'Personal Tips',text:'5 actionable tips tailored to your specific weaknesses.'},{icon:'📈',title:'Track Progress',text:'Analyze multiple times and watch your improvement over time.'}].map((f,i)=>(
          ...
          <div style={{maxWidth:760,margin:'0 auto',padding:'0 40px 90px',textAlign:'center'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(250px,1fr))',gap:18}}>
              <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:20,padding:'28px 24px',textAlign:'left' as any}}>
                <div style={{fontSize:12,fontWeight:700,color:G.muted,textTransform:'uppercase' as any,letterSpacing:'.1em',marginBottom:10}}>Free</div>
              ...
              <div style={{background:`linear-gradient(135deg,${G.accentSoft},rgba(139,92,246,.07))`,border:`1px solid ${G.accent}55`,borderRadius:20,padding:'28px 24px',textAlign:'left' as any,position:'relative'}}>
                ...
                <div style={{fontSize:12,fontWeight:700,color:G.accent,textTransform:'uppercase' as any,letterSpacing:'.1em',marginBottom:10}}>Pro</div>
                <div style={{display:'flex',alignItems:'baseline',gap:6,marginBottom:3}}><div style={{fontSize:38,fontWeight:900,fontFamily:'"Playfair Display",serif'}}>$9</div><div style={{color:G.muted,fontSize:13}}>/week</div></div>
      ...
      {screen==='record' && (
        <RecordScreen
          ...
          onTranscribeAndAnalyze={handleTranscribeAndAnalyze}
          ...
          <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:18,padding:24,width:'100%',maxWidth:380}}>
            {['Processing transcript…','Analyzing tone & pace…','Detecting patterns & errors…','Generating personal tips…'].map((step,i)=>(
              <div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'9px 0',borderBottom:i<3?`1px solid ${G.border}`:'none'}}>
                <div style={{width:22,height:22,borderRadius:'50%',border:`2px solid ${i<analysisStep?G.accent:G.border}`,background:i<analysisStep?G.accentSoft:'none',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,flexShrink:0,transition:'all .4s'}}>
          ...
          <div style={{maxWidth:880,margin:'0 auto',padding:'36px 20px 72px',display:'flex',flexDirection:'column',gap:20}} className="fadeUp">
            <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:22,padding:'32px 28px',display:'flex',alignItems:'center',gap:32,flexWrap:'wrap' as any}}>
              ...
              <div style={{flex:1,minWidth:220}}>
                <div style={{fontSize:12,fontWeight:700,color:G.accent,textTransform:'uppercase' as any,letterSpacing:'.1em',marginBottom:9}}>Overall Score</div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:18}}>
              <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:18,padding:22}}>
                <div style={{fontSize:11,fontWeight:700,color:G.muted,textTransform:'uppercase' as any,letterSpacing:'.08em',marginBottom:18}}>Category Breakdown</div>
              ...
              <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:18,padding:22}}>
                <div style={{fontSize:11,fontWeight:700,color:G.muted,textTransform:'uppercase' as any,letterSpacing:'.08em',marginBottom:18}}>Issues Found</div>
            ...
            <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:18,padding:24}}>
              <div style={{fontSize:11,fontWeight:700,color:G.muted,textTransform:'uppercase' as any,letterSpacing:'.08em',marginBottom:20}}>🎯 Your Personal Action Plan</div>
      ...
      {showAuth&&<AuthModal authTab={authTab} setAuthTab={(t:any)=>{setAuthTab(t);setAuthError('');}} authName={authName} setAuthName={setAuthName} authEmail={authEmail} setAuthEmail={setAuthEmail} authPassword={authPassword} setAuthPassword={setAuthPassword} authError={authError} authLoading={authLoading} onClose={()=>{setShowAuth(false);setAuthError('');}} onSignUp={handleSignUp} onSignIn={handleSignIn}/>}
      ...
      {showUpgrade&&(
        <Modal title="Unlock Pro" onClose={()=>setShowUpgrade(false)}>
          ...
          {[{plan:'weekly',label:'$9 / week',sub:'Auto-renews · cancel anytime'},{plan:'onetime',label:'$29 one-time',sub:'Lifetime access · no subscription',badge:'POPULAR'}].map(({plan,label,sub,badge}:any)=>(
